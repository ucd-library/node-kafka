...
class KafkaClient {
  ...
  constructor(type, globalConfig, topicConfig, opts={}) {
    ...
    if( type === 'consumer' ) {
      ...
      this.client = new Kafka.KafkaProducer(globalConfig, topicConfig);
    } else {
  ...
  _connect(opts, autoconnect) {
    this.client.connect(opts, (err, data) => {
      ...
      if( err ) {
        if( autoconnect ) {
          logger.error('Failed to connect to kafka, retrying', err);
          return this._connect(opts, autoconnect);
        }
        logger.fatal('Failed to connect to kafka', err);
